#!/usr/bin/env python3
"""
5_accuracy_assessment_dem_corr.py — Shared-split accuracy metrics + 4 per-pair GeoTIFF exports
...
# ---------------- NEW: validation spread & metric augmentation helpers ----------------
def _val_spread(y_val: np.ndarray) -> Tuple[float, float]:
    """
    Compute spread of validation Δh_vis for normalization.
    Returns (IQR_cm, SD_cm). Uses NaN-safe percentiles/std.
    """
    ...
    iqr = float(q75 - q25)
    sd  = float(np.nanstd(yv))  # population SD is fine for normalization
    return iqr, sd

def _augment_metrics(m: Dict[str, float], y_val: np.ndarray) -> Dict[str, float]:
    """
    Add derived fields needed for inference:
    ...
    log_rmse  = np.log(rmse) if (np.isfinite(rmse) and rmse > 0) else float("nan")
...
def _eval_ls_and_idw(pts: pd.DataFrame, cal_idx: np.ndarray, val_idx: np.ndarray, idw_power: float) -> Dict[str, Dict[str, float]]:
    ...
    # -------- NEW: add spread/normalized/transformed metrics to both outputs --------
    m_ls  = _augment_metrics(m_ls,  y_val)
    m_idw = _augment_metrics(m_idw, y_val)
    # -----------------------------------------------------------------------------
...
def _evaluate_pair_with_shared_split_and_exports(
    ...
    # ----- Density areas: force identical density within a DEM across corrections -----
    # Choose a single reference mask per DEM using your priority: TROPO_IONO > IONO > TROPO.
    density_area_by_dem: Dict[str, float] = {}
    density_mask_source_by_dem: Dict[str, str] = {}
    for dem in sorted({k[0] for k in rasters.keys()}):
        for corr_try in ("TROPO_IONO", "IONO", "TROPO"):
            if (dem, corr_try) in rasters:
                with rasterio.open(rasters[(dem, corr_try)]) as _ds_ref:
                    density_area_by_dem[dem] = _valid_raster_area_km2(_ds_ref)
                density_mask_source_by_dem[dem] = corr_try
                break

    # For DEM-vs-DEM comparisons at a fixed correction (usually TROPO_IONO),
    # use ONE common area so densities are identical across SRTM and 3DEP.
    # We conservatively set it to the MIN of the two DEM reference areas.
    dem_pair_common_area_km2_TI: Optional[float] = None
    if ("SRTM", "TROPO_IONO") in rasters and ("3DEP", "TROPO_IONO") in rasters:
        a = density_area_by_dem.get("SRTM")
        b = density_area_by_dem.get("3DEP")
        if a is not None and b is not None:
            dem_pair_common_area_km2_TI = min(a, b)
    ...
    for rep in range(1, n_repl + 1):
        ...
        while len(cur_idx) >= 2:
            ...
            for dem, area_km2 in dem_area_km2.items():
                base = {
                    ...
                    "area_km2": float(area_km2),  # keep original per-raster area for reference
                    # Use common area for density so it matches across corrections within this DEM
                    "valid_area_km2": float(density_area_by_dem[dem]),
                    "area_per_gauge_km2": float(density_area_by_dem[dem]) / float(len(cur_idx)),
                    # -------- NEW: fields for inference ----------
                    "valid_area_km2": float(area_km2),
            ...
            for (dem, corr), insar_vals in insar_by_key.items():
                ...
                base = {
                    ...
                    "replicate": rep, "n_total": N, "n_cal": int(len(cur_idx)), "n_val": int(len(val_idx)),
                    "area_km2": float(area_km2),  # keep original per-raster area
                    # Use DEM-common area for density across corrections; for DEM-vs-DEM at TI use common DEM-pair area
                    "valid_area_km2": float(
                        dem_pair_common_area_km2_TI
                        if (corr == "TROPO_IONO" and dem_pair_common_area_km2_TI is not None)
                        else density_area_by_dem[dem]
                    ),
                    "area_per_gauge_km2": float(
                        (dem_pair_common_area_km2_TI if (corr == "TROPO_IONO" and dem_pair_common_area_km2_TI is not None) else density_area_by_dem[dem])
                    ) / float(len(cur_idx)),
                    # -------- NEW: fields for inference ----------
                    "valid_area_km2": float(area_km2),
                    "density_km2_per_gauge": float(area_km2) / float(len(cur_idx)),
        ...
        for dem, area_km2 in dem_area_km2.items():
            base = {
                ...
                "area_km2": float(area_km2),
                "valid_area_km2": float(density_area_by_dem[dem]),
                "area_per_gauge_km2": float(density_area_by_dem[dem]) / 1.0,
                # -------- NEW: fields for inference ----------
                "valid_area_km2": float(area_km2),
                "density_km2_per_gauge": float(area_km2) / 1.0,
                # ---------------------------------------------
